...
@click.option(
    "--results", "-r", type=int, default=5, help="Maximum results [Default: 5]"
)
@click.option(
    "--base-url",
    "-u",
    ...
    help="Base url of readcomiconline " "[Default: https://readcomiconline.li]",
)
@click.pass_context
def search(ctx: click.core.Context, keyword: str, results: int, base_url: str) -> None:
    ...
    if not base_url.startswith("http"):
        click.secho(
            "Error: no scheme provided. Did you mean " f"https://{base_url}?", fg="red"
        )
        exit(1)
    ...
    soup, _ = page_downloader(
        urljoin(base_url, "Search/Comic"), data=data, method="POST"
    )
...
@click.argument("url")
@click.option(
    "--directory", "-d", default="downloads", help="The directory " "to download to"
)
@click.option(
    "--keep/--no-keep",
    default=True,
    help="Keep raw image files " "after converting to pdf [Default: True]",
)
@click.option(
    "--pdf/--no-pdf", default=True, help="Convert images to pdf " "[Default: True]"
)
@click.pass_context
def download(
    ctx: click.core.Context, url: str, directory: str, keep: bool, pdf: bool
) -> None:
    ...
    if len(url_split) in [5]:
        ...
        for chapter_link in all_links:
            ...
            try:
                ...
                click.secho(
                    "An exception ocurred while trying to download " f"{chapter_link}.",
                    fg="red",
                )
                traceback.print_exc()
        ...
        click.secho(f'Downloading "{comic_name}"...', fg="green")
        click.secho(
            "All HTML downloads will take some time, because we have "
            "to fool CloudFlare.",
            fg="yellow",
        )
