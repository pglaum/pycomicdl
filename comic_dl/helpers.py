...
def page_downloader(
    url: str, scraper_delay: int = 10, **kwargs: Any
    ...
    if method == "POST":
        ...
        connection = session.post(url, headers=headers, cookies=cookies, data=data)
    else:
        connection = session.get(url, headers=headers, cookies=cookies)

    if connection.status_code != 200:

        # anything other than 200 is unexpected and therefore wrong.
        click.secho("Whoops! Seems like I can not connect to the website.", fg="red")
...
def downloader(
    image_and_name: Tuple[str, str], referer: str, directory: str, **kwargs: Any
) -> None:
    ...
    if os.path.isfile(filepath):
        ...
        try:
            ...
            r = session.get(
                image_ddl, stream=True, headers=headers, cookies=kwargs.get("cookies")
            )
            r.raise_for_status()
        ...
        # catch all errors and print them out...
            ...
            pbar.write("A problem ocurred while downloading this image: " f"{filename}")
            pbar.write(e)
            raise
    ...
    if convert_to_pdf:
        ...
        try:
            if os.path.isfile(pdf_file):
                click.secho(f"PDF file exists! Skipping: {pdf_file}", fg="yellow")
            else:
...
def multithreaded_download(
    ...
    pbar.set_description(
        f"[comics.py] Downloading: {comic_name} " f"[{chapter_number}] "
    )
    ...
    try:
        ...
        pbar.set_description(
            f"[comics.py] Error: {comic_name} " f"[{chapter_number}] - {err}"
        )
        raise err
    except queue.Empty:
        pbar.set_description(f"[comics.py] Done: {comic_name} " f"[{chapter_number}] ")
        return
    finally:
        pbar.close()
...
def single_chapter(
    url: str, comic_name: str, directory: str, keep: bool, pdf: bool
) -> None:
    ...
    for current_chapter, image_link in enumerate(img_list):
        ...
        # highest quality
        image_link = image_link.replace("=s1600", "=s0").replace("/s1600", "/s0")
    ...
    multithreaded_download(
        chapter_number, comic_name, url, download_directory, filenames, links
    )
